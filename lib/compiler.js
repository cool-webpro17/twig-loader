...
module.exports = function(options) {
    return function(id, tokens, pathToTwig) {
        ...
        var pattern = /\\/;
        var templatesDeep = resourcePath.split('templates')[1].split(pattern).length - 2;
            ...
            if (token.type == "logic" && token.token.type) {
                switch(token.token.type) {
                    ...
                    case 'Twig.logic.type.include':
                        for (var i = 0; i < templatesDeep; i++) {
                            token.token.stack[0].value = '../' + token.token.stack[0].value;
                        }
